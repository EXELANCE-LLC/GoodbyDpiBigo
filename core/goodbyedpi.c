...
#define GOODBYEDPIBIGO_VERSION "v1.1.0"
...
int main(int argc, char *argv[]) {
    ...
    printf(
        "GoodbyeDpiBigo " GOODBYEDPIBIGO_VERSION
        ": Passive DPI blocker and Active DPI circumvention utility\n"
        "https://github.com/EXELANCE-LLC\n\n"
    );

    if (argc == 1) {
        /* Enable mode -5 with enhanced features for Windows apps */
        ...
        do_fake_packet = 1;
        do_auto_ttl = 1;
        do_wrong_seq = 1;          /* Add wrong sequence for better bypass */
        do_wrong_chksum = 1;       /* Add wrong checksum for better bypass */
        do_http_allports = 1;      /* Handle all ports for Windows apps */
        do_passivedpi = 1;         /* Enable passive DPI */
        max_payload_size = 1200;
        
        /* Enhanced DNS settings for Windows apps */
        do_dnsv4_redirect = 1;
        do_dnsv6_redirect = 1;
        do_dns_verb = 1;           /* Enable DNS debug messages */

        /* Set default DNS addresses and ports */
        if (inet_pton(AF_INET, "77.88.8.8", &dnsv4_addr) != 1) {
            puts("Error setting default IPv4 DNS address!");
            exit(ERROR_DNS_V4_ADDR);
        }
        dnsv4_port = htons(1253);
        
        if (inet_pton(AF_INET6, "2a02:6b8::feed:0ff", &dnsv6_addr) != 1) {
            puts("Error setting default IPv6 DNS address!");
            exit(ERROR_DNS_V6_ADDR);
        }
        dnsv6_port = htons(1253);

        /* Add port 53 to filter for DNS handling */
        add_filter_str(IPPROTO_UDP, 53);
        add_filter_str(IPPROTO_UDP, 1253);  // Add custom DNS port to filter

        printf("Starting with enhanced mode -5 and DNS settings:\n");
        printf("Mode features: fragment_http/https, reverse_frag, native_frag,\n");
        printf("              fake_packet, auto_ttl, wrong_seq, wrong_chksum\n");
        printf("DNS settings:\n");
        printf("  IPv4: 77.88.8.8:1253\n");
        printf("  IPv6: [2a02:6b8::feed:0ff]:1253\n");
        printf("Additional: all ports handling, passive DPI, DNS verbosity\n");
    ...
    while ((opt = getopt_long(argc, argv, "123456789pqrsaf:e:mwk:n", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'x': // --debug-exit
                ...
                break;
            default:
                puts("Usage: goodbyedpibigo.exe [OPTION...]\n"
                " -p          block passive DPI\n"
                " -q          block QUIC/HTTP3\n"
    ...
    printf("Filter activated, GoodbyeDpiBigo is now running!\n");
